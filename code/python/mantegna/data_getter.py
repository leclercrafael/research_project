...
class Data_Getter:
    ...
    def get_financials(self, tickers : list, start_date : str, end_date : str) -> pd.DataFrame :
        ...
        if os.path.exists('code/csv/'+ self.filename):
            self.data = pd.read_csv("code/csv/"+ self.filename, index_col=0, parse_dates=True)
        ...
        self.data.to_csv('code/csv/'+self.filename)
    ...
    def get_log_returns(self) -> pd.DataFrame:
        ...
        if os.path.exists('code/csv/log_returns_'+ self.filename):
            log_returns = pd.read_csv("code/csv/log_returns_"+ self.filename, index_col=0, parse_dates=True)
            return log_returns
        ...
        log_returns.to_csv('code/csv/log_returns_'+ self.filename)
