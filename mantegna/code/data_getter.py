...
class Data_Getter:
    ...
    def get_tickers(self, get_adr : bool = True, kg : bool = True) -> list :
        ...
        if kg : 
            ...
            df['Tickers'] = df['Tickers'].str.replace('"', '', regex=False).str.replace(',', '', regex=False).str.strip()
    ...
    def get_financials(self, tickers : list, start_date : str, end_date : str) -> pd.DataFrame :
        ...
        if self.kg:
            self.filename = f'kg_companies_{start_date}_{end_date}.csv'
        elif self.get_adr :
